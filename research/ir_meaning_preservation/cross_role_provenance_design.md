...
この文書は、`CHECK`, `FILTER`, `CALCULATE` を横断して provenance metadata をどう統一的に扱うかを定義するためのものである。
...
- `role_source` -> implicit upstream data
- `role_provenance` -> `entity_resolution`
...
3. `CHECK`, `FILTER`, `CALCULATE` は field 名は異なっても、同一の「resolved value + provenance + policy」枠組みで説明できる
