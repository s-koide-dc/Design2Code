...
4. **自動チェーン挿入（`JSON_DESERIALIZE` 等）の妥当性評価**
   - 初期論点として挙げられていたが、本フェーズでは対象外とし今後の課題とした。
5. **IR を共通中間表現とすることによる多言語生成への一般化**
   - 本フェーズでは単一言語・基盤レベルでの意味保存に注力したため、多言語化への貢献については今後の課題とした。
