...
1.  **ActionExecutor連携**: `ActionExecutor`インスタンスを参照し、`src.utils.action_intents` の `INTENT_CMD_RUN` に対してコマンド名が `safe_commands` に含まれるか検証する。
...
7.  **破壊的操作の判定**: 削除 (`INTENT_FILE_DELETE`)、移動 (`INTENT_FILE_MOVE`)、コード修正 (`INTENT_APPLY_CODE_FIX`, `INTENT_APPLY_REFACTORING`) などの意図を**高リスク (HIGH)** として判定し、承認を要求する。
8.  **バックアップ付き削除**: `INTENT_BACKUP_AND_DELETE` は高リスクとして扱う。
9.  **注意が必要な操作の判定**: コマンド実行 (`INTENT_CMD_RUN`)、ファイル追記 (`INTENT_FILE_APPEND`)、新規作成 (`INTENT_FILE_CREATE`) などを**高リスク (HIGH)** または **中リスク (MEDIUM)** として判定する。
