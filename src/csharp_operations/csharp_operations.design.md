...
- **Core Logic**:
    ...
    2. 対象プロジェクトのC#・プロジェクト設定ファイルとAnalyzer自身の内容からSHA-256 fingerprintを生成します。fingerprint は対象 root のインデックスと root 相対パス、ファイル内容で構成し、絶対ドライブ名や workspace root の違いで不必要に変化しないようにします。
...
- 2026-07-08: 解析 fingerprint を root 相対パスと内容に基づけ、ドライブ差異や workspace root 差異で cache key が揺れない契約を反映。
