...
3.  **安全な分析 (`_safe_analyze_file`)**: 
    ...
    - **エラー分離**: 個別の検出器の例外が全体の分析を停止させないよう保護し、結果を集約する。失敗は `analysis_diagnostics` にファイル、操作、例外型、検出器名を記録する。
    - **ファイル診断**: stat/read/decode失敗と10MB超過は空のスメル一覧と区別し、診断へ記録する。
...
## 4. Review Notes
- 2026-06-30: ファイル・検出器単位の部分失敗診断と限定例外処理を反映。
