...
### Output
- **Description**: `clarification_needed`、`dialogue_state`、`response.text` を更新したコンテキスト。
- **Type/Format**: `Dict[str, Any]`

### Core Logic
1. 起動時にテンプレートを `resources/knowledge_base.json` から読み込み、未存在時はデフォルトテンプレートを使用する。
2. `clarification_needed` が既に True の場合は、`task.clarification_message` を優先して返し、`dialogue_state` が未設定なら `task_clarification` を補完する。
...
9. 生成したメッセージを `response.text` に設定し、`clarification_needed` を True、`dialogue_state` を `task_clarification` にする。
10. `dialogue_state` がすでに `pending_confirmation` なら上書きせず、承認待ちと通常補足質問の責務を混在させない。
11. 付属の `DummyActionExecutor` と手動確認ケースでも `src.utils.action_intents` の共通定数を使い、必須エンティティ判定サンプルの intent 名直書きを避ける。
...
## 4. Operational Notes
- `DummyLogManager` と `__main__` の手動確認ケースは `src.utils.stdout_guard.debug_print` による opt-in 出力とする。
- Clarification 本体の `response.text` 契約は維持し、通常のパイプライン実行では追加の stdout を出さない。
