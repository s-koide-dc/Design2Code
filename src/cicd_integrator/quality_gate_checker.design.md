...
1.  **メトリクスの収集 (`_collect_metrics`)**: 
    ...
    - 複数globに一致する同一ファイルは一度だけ解析する。
    - 壊れたテスト結果は `test_result_errors`、壊れたカバレッジ結果は `coverage_result_errors` にファイルパスと例外型を記録する。解析失敗を「結果ファイルなし」と同一視しない。
2.  **ゲート評価**: 
...
## 5. Review Notes
- 2026-06-29: 結果ファイル解析の診断契約、例外型限定、重複ファイル排除を反映。
