...
- **診断可能性**: 存在する設定のJSON破損、OS読込エラー、トップレベル型不正は `load_errors` にパスと例外型だけを記録する。設定内容と例外本文は保持しない。
- **欠落との区別**: 存在しない任意設定は `missing_config_files` に記録し、破損設定とは区別する。
- **Strict Mode**: `ConfigManager(strict=True)` は `load_errors` が1件以上あれば `ConfigLoadError` を送出し、不正設定のまま起動しない。
...
- **Fail-closed option**: 検証・デプロイ用途ではstrict modeにより存在する不正設定を拒否可能
- **明確な分離**: 設定ファイル（`config/`）とデータアセット（`resources/`）を明確に分離
- **一貫性**: 全てのモジュールが同じインターフェースで設定にアクセス

## 8. Review Notes
- 2026-06-29: 設定欠落と破損の区別、非機密診断、strict modeの起動拒否契約を反映。
