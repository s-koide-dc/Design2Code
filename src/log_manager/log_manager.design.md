...
- JSON ログは1行1イベントの JSON Lines として書き込み、読取時は旧形式の配列 / 単一オブジェクト / 行末カンマ付きレコードも互換的に扱う。
- ログ読取時の不正レコード、非オブジェクト、timestamp 不備は標準出力へ出さず、`logging.getLogger(__name__)` 経由の補助診断として記録する。
- 設定ファイル指定時は `sensitive_fields` と `sensitive_field_keywords` を設定値で上書きし、未指定時はデフォルトの固定フィールドと keyword 群を使う。
...
3. **パース処理**:
   - 現行形式は1行1イベントの JSON Lines とする。
   - 旧形式の行末カンマ付きレコードも行単位で `json.loads` し、互換読取する。
   - 不正レコードは行番号付きの警告を記録して除外する。
   - `timestamp` が欠落または不正なイベントも診断後に除外する。
...
- `get_events_after` はメモリ内イベント履歴ではなく現在の JSON ログファイルを読むため、プロセスをまたいだイベント収集に使える。
