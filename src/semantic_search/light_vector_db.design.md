...
3. `upsert` で ID をキーにメタデータとベクトルを更新・追加する。保存は保存先パス単位のプロセス内ロックで直列化し、同一ディレクトリ内の一時ファイルをfsyncしてから `os.replace` する。Windowsの短時間ロック競合には段階的な短時間リトライを行い、複数インスタンスの同時更新と途中書き込みによる破損を避ける。
4. `query` は内積を計算し、上位 `top_k` 件を返す（正規化済みベクトル前提）。
5. `remove` で指定 ID を削除し、インデックスを再構築する。最後の要素を削除してベクトルが空になった場合は、古い `.npy` ファイルも削除する。
