...
このうちプロダクトとして保証する主経路は設計書からC#を生成する
フローである。対話・意味検索・複数ファイル生成は実験的能力であり、
資産要件と昇格条件は [product_scope.md](./product_scope.md) を参照する。
...
- 設計書生成フロー: Design Parser → IR Generator → Code Synthesis → CodeBuilder
  ...
  - 補足: 標準回帰の既定セットは `ComplexLinqSearch` / `ConjunctiveLinqSearch` / `ExplicitConditionBranch` / `CsvSalesAggregation` / `ProductApiFilteredCatalog` / `CustomerApiWithEntitySpec` / `DailyInventorySync` / `SecureOrderProcessing` / `StateUpdatePersist` / `AppModeEchoMinimal` / `RobustConfigLoader` / `StdinToStdoutTransform` / `AggregationSummary` / `SyncExternalData`。
  - 補足: `smoke` プロファイルは JSON/LINQ、HTTP、SQLite更新、設定ファイル分岐の代表4件を短時間で確認し、`quality` プロファイルは全14件と明示runtime oracleを確認する。
...
- 生成結果の単発レビュー: `scripts/design/review_design_generation_snapshot.py`
- 生成結果の標準回帰: `scripts/design/run_design_generation_regression.py`
