# Project Architecture: ローカル設計書駆動C#生成器と実験的対話基盤

本ドキュメントは、プロジェクト全体の構造、モジュール間の依存関係、およびデータフローを俯瞰するためのアーキテクチャ・マップです。プロダクトとしての主経路は設計書からC#を生成するフローです。対話応答・検索・自律学習は実験的能力として併設されており、保証範囲は [docs/product_scope.md](./docs/product_scope.md) を正本とします。
