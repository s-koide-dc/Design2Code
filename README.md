...
- CI では `.github/workflows/python-ci.yml` の `generation-quality` ジョブで CodeBuilder を事前ビルドし、成功台帳に登録した全シナリオの品質ゲートと実行時挙動を必須確認します。台帳再生は `--fail-on-maintainability --run-runtime-oracles --require-runtime-oracles --summary-only` 相当で実行し、CodeBuilder/Roslyn の AST 解析で取得した保守性しきい値超過、明示 oracle 不足、または oracle 失敗を失敗扱いにします。workflow は push / pull request に加え、GitHub Actions 画面から `workflow_dispatch` で手動再実行できます。
...
- 現在の既定回帰セットは `ComplexLinqSearch` / `ConjunctiveLinqSearch` / `DisjunctiveLinqSearch` / `ExplicitConditionBranch` / `CsvSalesAggregation` / `ProductApiFilteredCatalog` / `CustomerApiWithEntitySpec` / `DailyInventorySync` / `SecureOrderProcessing` / `StateUpdatePersist` / `AppModeEchoMinimal` / `RobustConfigLoader` / `StdinToStdoutTransform` / `AggregationSummary` / `SyncExternalData` です。
- `--profile smoke` は `ComplexLinqSearch` / `ProductApiFilteredCatalog` / `StateUpdatePersist` / `RobustConfigLoader` の生成・静的品質を短時間で確認します。`--profile quality`（既定）は `resources/supported_generation_designs.json` の検証済み単一モジュール全件を対象にします。`--require-runtime-oracles` を付けると、全Test Caseが有効な明示oracleを持ち、その全件が実行成功しなければ失敗します。CIでは smoke と、成功台帳を正本にした完全再生を独立ジョブとして並列実行します。
